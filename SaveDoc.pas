...
type
  ...
  public
    ...
    procedure LoadMatter(var AFoundFileID: string; var ANMatter: integer; AFileID: string);
  end;
...
uses
   LoginDetails, SaveDocFunc, MatterSearch, GenericSearch;
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  qryGetEntity.ParamByName('Emp').AsString := TableString('EMPLOYEE','USER_NAME', UserID, 'CODE');
...
procedure TdmSaveDoc.LoadMatter(var AFoundFileID: string; var ANMatter: integer; AFileID: string);
var
  LFileID,
  LFileIDFilter: String;
  bSuccess: boolean;
  frmGenericSearch: TfrmGenericSearch;
begin
   LFileID := Uppercase(trim(AFileID));
   bSuccess := False;
   with qryNew do
   begin
      try
         Close;
         SQL.Text := 'SELECT matter.fileid as code, phonebook.search||'' ''||matter.shortdescr as descr, ''N'' AS DEFAULTITEM, nmatter '+
                     'FROM phonebook, matter WHERE matter.nclient = phonebook.nclient ';

         SQL.Text := SQL.Text + 'and matter.entity = '+ QuotedStr(Entity) ;

         if (Trim(LFileID).IsEmpty = False) then
         begin
            LFileIDFilter := StringReplace(LFileID, '/', '\/',[rfReplaceAll, rfIgnoreCase]);
            LFileIDFilter := StringReplace(LFileIDFilter, '-', '\-',[rfReplaceAll, rfIgnoreCase]);
            LFileIDFilter := '%' + LFileIDFilter + '%';
            SQL.Text := SQL.Text + ' and contains(matter.dummy,'+ QuotedStr(trim(LFileIDFilter)) +') > 0';
         end;
      finally
//         Prepare;
         Open;
      ...
      begin
         ...
         if (Trim(LFileID) <> '') then
            SQL.Text := SQL.Text + ' and upper(search) like '+ QuotedStr(LFileIDFilter);
         Prepare;
         Open;
      end;
      ...
      begin
         try
            ...
            begin
               ...
               ANMatter := frmGenericSearch.qrySource.FieldByName('NMATTER').AsInteger;
            end
            else
               LFileID := '';
         finally
      ...
      begin
         ...
         begin
            MsgErr('The selected Matter could not be found.  Please check and re-try.');
            LFileID := '';
            Exit;
         end;
      end;
